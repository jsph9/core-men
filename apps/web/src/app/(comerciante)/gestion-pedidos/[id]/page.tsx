...
export default function DetalleCotizacion() {
  ...
  const [activeModal, setActiveModal] = useState<"RECHAZAR" | "ACEPTAR" | "WHATSAPP" | "VIABLE" | "INVIABLE" | "CANCEL_ORDER" | "UPDATE_ORDER" | null>(null);
  ...
  if (!quote) return <div className="p-8 text-center text-red-500 flex flex-col h-64 items-center justify-center gap-2">
    ...
    <Link href="/gestion-pedidos" className="mt-4 text-blue-500 underline">Volver a la bandeja</Link>
  </div>;
      ...
      <div className="flex-1 pb-8">
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-6 mb-6 flex flex-col gap-6 shadow-sm mx-6 mt-6">
          ...
          <div className="flex items-center gap-4">
            <Link href="/gestion-pedidos" className="p-2 bg-slate-100 hover:bg-slate-200 rounded-md text-slate-600 transition-colors">
              <ChevronLeft className="h-5 w-5" />
            </Link>
            <div>
              <p className="text-xs text-slate-500 mb-0.5">Pedidos {'>'} <span className="font-semibold text-slate-900">Pedido #{id?.slice(0, 6).toUpperCase()}</span></p>
              <h1 className="text-2xl font-bold text-[#0F172A]">Detalle del Pedido</h1>
            </div>
          </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-6">
                  ...
                  <div className="bg-slate-900 rounded-lg p-4 text-white shadow-sm border border-slate-800">
                    <div className="flex items-center gap-1.5 mb-1">
                      ...
                      <p className="text-[10px] text-blue-300 font-semibold tracking-wider">VALOR FINAL</p>
                    </div>
                    <div className="flex items-baseline gap-1">
                      <span className="text-3xl font-bold text-white whitespace-nowrap">
                        S/ {Number(quote?.finalPrice || quote?.customerPrice || quote?.estimatedPrice || 0).toFixed(2)}
                      </span>
                    </div>
                  </div>
                </div>
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="py-4">
            ...
            <Textarea 
              ...
              onChange={(e) => {
                // To avoid creating a new state, we can reuse unfeasibleReason or rejectReason state from the original file
          ...
          <DialogFooter className="gap-2 sm:gap-0">
            ...
            <Button 
              ...
              onClick={() => cancelOrder.mutate({ rejectionReason: rejectReason })}
              disabled={cancelOrder.isPending} // requires rejectReason
            >
          ...
          <div className="py-2 space-y-4">
            ...
            <div className="bg-slate-50 border border-slate-100 rounded-xl p-4 text-xs space-y-2">
              <div className="flex justify-between items-center text-slate-500">
                ...
                <span className="font-bold text-slate-800">S/ {Number(quote?.finalPrice || quote?.customerPrice || quote?.estimatedPrice || 0).toFixed(2)}</span>
              </div>
              <div className="flex justify-between items-center text-slate-500">
